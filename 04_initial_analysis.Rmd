...
flextable::save_as_docx(
  path = "output/table/splitted_stat_desc.docx", 
  prop_section = officer::prop_section(
    page_size = officer::page_size(
      orient = "landscape", # Keep the orientation as landscape 
      width = 10,          # Width aligned to fit a PowerPoint slide
      height = 5.63        # Height for 16:9 aspect ratio
    ),
    type = "continuous", 
  ...
  # as_gt() %>% 
  # gt::as_word() %>% 
  # gt::gtsave(filename = "output/table/splitted_stat_desc.docx")
```
...
stargazer(re0, re1, re2, re3, re4, re5, type = "html", title = "Splitted hh. characteristics at baseline", digits=2, float = TRUE, se.position = "below", omit.stat=c("LL","ser","f","adj.rsq","res.dev"), dep.var.labels.include = TRUE,font.size="small",
se.position = "below",notes.align = "l",table.placement = "H",header = F, column.labels = c("","Exposed UP.", "At least One", "Highly exposed", "Speed","Duration"),
keep = c("hhm_sex" , "hhm_age", "hh_size" ,  "marital_status_hhm", "literacy_hhm", "education_high","hh_head_religion","hh_ethnic_group", "mean_speed_roanu", "mean_time_roanu", "affected_roanu","high_exposed_roanu","exposed_at_least_zero_roanu","affected_roanu","nbr_female", "nbr_underfive", "nbr_yngchldrn_5_10", "nbr_teenager_10_20", "nbr_adults_20_65", "nbr_female_underfive", "nbr_female_yngchldrn_5_10", "nbr_female_teenager_10_20", "nbr_female_adults_20_65"), out = paste0("output/table/splitted_reg_roanu.html", sep = ""))
...
stargazer(re0, re1, re2, re3, re4, re5, type = "html", title = "Splitted hh. after the disaster characteristics at baseline", digits=2, float = TRUE, se.position = "below", omit.stat=c("LL","ser","f","adj.rsq","res.dev"), dep.var.labels.include = TRUE,font.size="small",
se.position = "below",notes.align = "l",table.placement = "H",header = F, column.labels = c("","Exposed UP.", "At least One", "Highly exposed", "Speed","Duration"),
keep = c("hhm_sex" , "hhm_age", "hh_size" ,  "marital_status_hhm", "literacy_hhm", "education_high","hh_head_religion","hh_ethnic_group", "mean_speed_roanu", "mean_time_roanu", "affected_roanu","high_exposed_roanu","exposed_at_least_zero_roanu","affected_roanu","nbr_female", "nbr_underfive", "nbr_yngchldrn_5_10", "nbr_teenager_10_20", "nbr_adults_20_65", "nbr_female_underfive", "nbr_female_yngchldrn_5_10", "nbr_female_teenager_10_20", "nbr_female_adults_20_65"),  out = paste0("output/table/splitted_reg_aft_roanu.html", sep = ""))
...
df_r1_hh %>% 
  ...
  # Selecting relevant variables
   select(hh_id, affected_komen,mean_speed_komen,mean_time_komen,hh_size,hh_head_religion,hh_ethnic_group) %>% 
  left_join(
    df_r1_anthr1 %>% 
      filter(relation_hhh=="primary respondent") %>% 
      select( hh_id, hhm_sex, hhm_age, marital_status_hhm, literacy_hhm, education_high, hhm_status, nbr_female, nbr_underfive, nbr_yngchldrn_5_10, nbr_teenager_10_20, nbr_adults_20_65, nbr_female_underfive, nbr_female_yngchldrn_5_10, nbr_female_teenager_10_20, nbr_female_adults_20_65)) %>% 
mutate(
  ...
  select(-c(hh_id)) %>% 
    ...
    missing = "ifany"
       ...
       add_p(test=list(all_continuous() ~ "t.test", all_categorical() ~ "fisher.test.simulate.p.values")) %>% 
  modify_header(label ~ "Affected union komen (Yes=1, No=0)") %>% 
  add_overall(last=TRUE) %>% 
  as_flex_table() %>% 
flextable::save_as_docx(
  path = "output/table/komen/baseline_by_aff_stat_desc.docx", 
  prop_section = officer::prop_section(
    page_size = officer::page_size(
      orient = "landscape", # Keep the orientation as landscape 
      width = 10,          # Width aligned to fit a PowerPoint slide
...
df_r1_hh %>% 
  mutate(hh_split_r2_r3 = ifelse(hh_split_r3=="1"| hh_split_r2=="1","1","0")) %>% 
    filter(hh_split_r2_r3=="0") %>% 
  # Selecting relevant variables
   select(hh_id,exposed_at_least_zero_komen,mean_speed_komen,mean_time_komen,hh_size,hh_head_religion,hh_ethnic_group) %>% 
  left_join(
    df_r1_anthr1 %>% 
      filter(relation_hhh=="primary respondent") %>% 
      select( hh_id, hhm_sex, hhm_age, marital_status_hhm, literacy_hhm, education_high, hhm_status, nbr_female, nbr_underfive, nbr_yngchldrn_5_10, nbr_teenager_10_20, nbr_adults_20_65, nbr_female_underfive, nbr_female_yngchldrn_5_10, nbr_female_teenager_10_20, nbr_female_adults_20_65)) %>% 
mutate(
  ...
  select(-c(hh_id)) %>% 
    ...
    missing = "ifany"
       ...
       add_p(test=list(all_continuous() ~ "t.test", all_categorical() ~ "fisher.test.simulate.p.values")) %>% 
  modify_header(label ~ "Exposed at least zero union komen (Yes=1, No=0)") %>% 
  add_overall(last=TRUE) %>% 
  as_flex_table() %>% 
flextable::save_as_docx(
  path = "output/table/komen/baseline_by_aff_at_least_zero_stat_desc.docx", 
  prop_section = officer::prop_section(
    page_size = officer::page_size(
      orient = "landscape", # Keep the orientation as landscape 
      width = 10,          # Width aligned to fit a PowerPoint slide
...
df_r1_hh %>% 
  mutate(hh_split_r2_r3 = ifelse(hh_split_r3=="1"| hh_split_r2=="1","1","0")) %>% 
    filter(hh_split_r2_r3=="0") %>% 
  # Selecting relevant variables
   select(hh_id, high_exposed_komen,mean_speed_komen,mean_time_komen,hh_size,hh_head_religion,hh_ethnic_group) %>% 
  left_join(
    df_r1_anthr1 %>% 
      filter(relation_hhh=="primary respondent") %>% 
      select( hh_id, hhm_sex, hhm_age, marital_status_hhm, literacy_hhm, education_high, hhm_status, nbr_female, nbr_underfive, nbr_yngchldrn_5_10, nbr_teenager_10_20, nbr_adults_20_65, nbr_female_underfive, nbr_female_yngchldrn_5_10, nbr_female_teenager_10_20, nbr_female_adults_20_65)) %>% 
mutate(
  ...
  select(-c(hh_id)) %>% 
    ...
    missing = "ifany"
       ...
       add_p(test=list(all_continuous() ~ "t.test", all_categorical() ~ "fisher.test.simulate.p.values")) %>% 
  modify_header(label ~ "Highly exposed union komen (Yes=1, No=0)") %>% 
  add_overall(last=TRUE) %>% 
  as_flex_table() %>% 
flextable::save_as_docx(
  path = "output/table/komen/baseline_by_highly_stat_desc.docx", 
  prop_section = officer::prop_section(
    page_size = officer::page_size(
      orient = "landscape", # Keep the orientation as landscape 
      width = 10,          # Width aligned to fit a PowerPoint slide
...
df_r1_hh %>% 
  mutate(hh_split_r2_r3 = ifelse(hh_split_r3=="1"| hh_split_r2=="1","1","0")) %>% 
    filter(hh_split_r2_r3=="0") %>% 
  # Selecting relevant variables
   select(hh_id, affected_roanu,mean_speed_roanu,mean_time_roanu,hh_size,hh_head_religion,hh_ethnic_group) %>% 
  left_join(
    df_r1_anthr1 %>% 
      filter(relation_hhh=="primary respondent") %>% 
      select( hh_id, hhm_sex, hhm_age, marital_status_hhm, literacy_hhm, education_high, hhm_status, nbr_female, nbr_underfive, nbr_yngchldrn_5_10, nbr_teenager_10_20, nbr_adults_20_65, nbr_female_underfive, nbr_female_yngchldrn_5_10, nbr_female_teenager_10_20, nbr_female_adults_20_65)) %>% 
mutate(
  ...
  select(-c(hh_id)) %>% 
    ...
    missing = "ifany"
       ...
       add_p(test=list(all_continuous() ~ "t.test", all_categorical() ~ "fisher.test.simulate.p.values")) %>% 
  modify_header(label ~ "Affected union roanu (Yes=1, No=0)") %>% 
  add_overall(last=TRUE) %>% 
  as_flex_table() %>% 
flextable::save_as_docx(
  path = "output/table/roanu/baseline_by_aff_stat_desc.docx", 
  prop_section = officer::prop_section(
    page_size = officer::page_size(
      orient = "landscape", # Keep the orientation as landscape 
      width = 10,          # Width aligned to fit a PowerPoint slide
...
df_r1_hh %>% 
  mutate(hh_split_r2_r3 = ifelse(hh_split_r3=="1"| hh_split_r2=="1","1","0")) %>% 
    filter(hh_split_r2_r3=="0") %>% 
  # Selecting relevant variables
   select(hh_id,exposed_at_least_zero_roanu,mean_speed_roanu,mean_time_roanu,hh_size,hh_head_religion,hh_ethnic_group) %>% 
  left_join(
    df_r1_anthr1 %>% 
      filter(relation_hhh=="primary respondent") %>% 
      select( hh_id, hhm_sex, hhm_age, marital_status_hhm, literacy_hhm, education_high, hhm_status, nbr_female, nbr_underfive, nbr_yngchldrn_5_10, nbr_teenager_10_20, nbr_adults_20_65, nbr_female_underfive, nbr_female_yngchldrn_5_10, nbr_female_teenager_10_20, nbr_female_adults_20_65)) %>% 
mutate(
  ...
  select(-c(hh_id)) %>% 
    ...
    missing = "ifany"
       ...
       add_p(test=list(all_continuous() ~ "t.test", all_categorical() ~ "fisher.test.simulate.p.values")) %>% 
  modify_header(label ~ "Exposed at least zero union roanu (Yes=1, No=0)") %>% 
  add_overall(last=TRUE) %>% 
  as_flex_table() %>% 
flextable::save_as_docx(
  path = "output/table/roanu/baseline_by_aff_at_least_zero_stat_desc.docx", 
  prop_section = officer::prop_section(
    page_size = officer::page_size(
      orient = "landscape", # Keep the orientation as landscape 
      width = 10,          # Width aligned to fit a PowerPoint slide
...
df_r1_hh %>% 
  mutate(hh_split_r2_r3 = ifelse(hh_split_r3=="1"| hh_split_r2=="1","1","0")) %>% 
    filter(hh_split_r2_r3=="0") %>% 
  # Selecting relevant variables
   select(hh_id, high_exposed_roanu,mean_speed_roanu,mean_time_roanu,hh_size,hh_head_religion,hh_ethnic_group) %>% 
  left_join(
    df_r1_anthr1 %>% 
      filter(relation_hhh=="primary respondent") %>% 
      select( hh_id, hhm_sex, hhm_age, marital_status_hhm, literacy_hhm, education_high, hhm_status, nbr_female, nbr_underfive, nbr_yngchldrn_5_10, nbr_teenager_10_20, nbr_adults_20_65, nbr_female_underfive, nbr_female_yngchldrn_5_10, nbr_female_teenager_10_20, nbr_female_adults_20_65)) %>% 
mutate(
  ...
  select(-c(hh_id)) %>% 
    ...
    missing = "ifany"
       ...
       add_p(test=list(all_continuous() ~ "t.test", all_categorical() ~ "fisher.test.simulate.p.values")) %>% 
  modify_header(label ~ "Highly exposed union roanu (Yes=1, No=0)") %>% 
  add_overall(last=TRUE) %>% 
  as_flex_table() %>% 
flextable::save_as_docx(
  path = "output/table/roanu/baseline_by_highly_stat_desc.docx", 
  prop_section = officer::prop_section(
    page_size = officer::page_size(
      orient = "landscape", # Keep the orientation as landscape 
      width = 10,          # Width aligned to fit a PowerPoint slide
...
df_r1_hh %>% 
  mutate(hh_split_r2_r3 = ifelse(hh_split_r3=="1"| hh_split_r2=="1","1","0")) %>% 
    filter(hh_split_r2_r3=="0") %>% 
  # Selecting relevant variables
   select(hh_id, affected_mora,mean_speed_mora,mean_time_mora,hh_size,hh_head_religion,hh_ethnic_group) %>% 
  left_join(
    df_r1_anthr1 %>% 
      filter(relation_hhh=="primary respondent") %>% 
      select( hh_id, hhm_sex, hhm_age, marital_status_hhm, literacy_hhm, education_high, hhm_status, nbr_female, nbr_underfive, nbr_yngchldrn_5_10, nbr_teenager_10_20, nbr_adults_20_65, nbr_female_underfive, nbr_female_yngchldrn_5_10, nbr_female_teenager_10_20, nbr_female_adults_20_65)) %>% 
mutate(
  ...
  select(-c(hh_id)) %>% 
    ...
    missing = "ifany"
       ...
       add_p(test=list(all_continuous() ~ "t.test", all_categorical() ~ "fisher.test.simulate.p.values")) %>% 
  modify_header(label ~ "Affected union mora (Yes=1, No=0)") %>% 
  add_overall(last=TRUE) %>% 
  as_flex_table() %>% 
flextable::save_as_docx(
  path = "output/table/mora/baseline_by_aff_stat_desc.docx", 
  prop_section = officer::prop_section(
    page_size = officer::page_size(
      orient = "landscape", # Keep the orientation as landscape 
      width = 10,          # Width aligned to fit a PowerPoint slide
...
df_r1_hh %>% 
  mutate(hh_split_r2_r3 = ifelse(hh_split_r3=="1"| hh_split_r2=="1","1","0")) %>% 
    filter(hh_split_r2_r3=="0") %>% 
  # Selecting relevant variables
   select(hh_id,exposed_at_least_zero_mora,mean_speed_mora,mean_time_mora,hh_size,hh_head_religion,hh_ethnic_group) %>% 
  left_join(
    df_r1_anthr1 %>% 
      filter(relation_hhh=="primary respondent") %>% 
      select( hh_id, hhm_sex, hhm_age, marital_status_hhm, literacy_hhm, education_high, hhm_status, nbr_female, nbr_underfive, nbr_yngchldrn_5_10, nbr_teenager_10_20, nbr_adults_20_65, nbr_female_underfive, nbr_female_yngchldrn_5_10, nbr_female_teenager_10_20, nbr_female_adults_20_65)) %>% 
mutate(
  ...
  select(-c(hh_id)) %>% 
    ...
    missing = "ifany"
       ...
       add_p(test=list(all_continuous() ~ "t.test", all_categorical() ~ "fisher.test.simulate.p.values")) %>% 
  modify_header(label ~ "Exposed at least zero union mora (Yes=1, No=0)") %>% 
  add_overall(last=TRUE) %>% 
  as_flex_table() %>% 
flextable::save_as_docx(
  path = "output/table/mora/baseline_by_aff_at_least_zero_stat_desc.docx", 
  prop_section = officer::prop_section(
    page_size = officer::page_size(
      orient = "landscape", # Keep the orientation as landscape 
      width = 10,          # Width aligned to fit a PowerPoint slide
...
df_r1_hh %>% 
  mutate(hh_split_r2_r3 = ifelse(hh_split_r3=="1"| hh_split_r2=="1","1","0")) %>% 
    filter(hh_split_r2_r3=="0") %>% 
  # Selecting relevant variables
   select(hh_id, high_exposed_mora,mean_speed_mora,mean_time_mora,hh_size,hh_head_religion,hh_ethnic_group) %>% 
  left_join(
    df_r1_anthr1 %>% 
      filter(relation_hhh=="primary respondent") %>% 
      select( hh_id, hhm_sex, hhm_age, marital_status_hhm, literacy_hhm, education_high, hhm_status, nbr_female, nbr_underfive, nbr_yngchldrn_5_10, nbr_teenager_10_20, nbr_adults_20_65, nbr_female_underfive, nbr_female_yngchldrn_5_10, nbr_female_teenager_10_20, nbr_female_adults_20_65)) %>% 
mutate(
  ...
  select(-c(hh_id)) %>% 
    ...
    missing = "ifany"
       ...
       add_p(test=list(all_continuous() ~ "t.test", all_categorical() ~ "fisher.test.simulate.p.values")) %>% 
  modify_header(label ~ "Highly exposed union mora (Yes=1, No=0)") %>% 
  add_overall(last=TRUE) %>% 
  as_flex_table() %>% 
flextable::save_as_docx(
  path = "output/table/mora/baseline_by_highly_stat_desc.docx", 
  prop_section = officer::prop_section(
    page_size = officer::page_size(
      orient = "landscape", # Keep the orientation as landscape 
      width = 10,          # Width aligned to fit a PowerPoint slide
...
df_r1_hh %>% 
  ...
  as_flex_table() %>% 
flextable::save_as_docx(
  path = "output/table/attrition_desc.docx", 
  prop_section = officer::prop_section(
    page_size = officer::page_size(
      orient = "landscape", # Keep the orientation as landscape 
      width = 10,          # Width aligned to fit a PowerPoint slide
...
ggsave("output/img/stat desc/komen/attrition_panelview_komen.jpeg", width = 18, height = 7, units = "cm")
...
lst_affected_roanu <- df_r1_hh %>% 
  filter(affected_roanu=="1") %>%  
  select(hh_id) %>% 
  pull()
...
# Use with `right_join()` to fill in missing rows (in data)
all_comb <- df_r1_hh %>%
          select(hh_id,survey_round, hh_size, affected_roanu,treat_group_roanu, survey_year) %>% 
          filter(hh_id %in% lst_menage)%>% 
              mutate(treat_group_roanu_post=0) %>% 
          plyr::rbind.fill(
            df_r2_hh %>%
              select(hh_id,survey_round, hh_size, affected_roanu,treat_group_roanu, survey_year)%>% 
          filter(hh_id %in% lst_menage)%>% 
          ...
          plyr::rbind.fill(
            df_r3_hh %>%
              select(hh_id,survey_round, hh_size, affected_roanu,treat_group_roanu, survey_year)%>% 
              filter(hh_id %in% lst_menage) %>% 
             #mutate(treat_group_roanu_post=treat_group_roanu))%>%
              mutate(treat_group_roanu_post=treat_group_roanu))%>% 
              #mutate(treat_group_roanu_post=ifelse(affected_roanu==treat_group_roanu))%>% 
  dplyr::full_join(all_comb, by=c("survey_round", "hh_id")) %>% 
  mutate(
    survey_year = ifelse(survey_round==1,2011,
                              ifelse(survey_round==2,2015,
                                     ifelse(survey_round==3,2018,NA))),
    treat_group_roanu = ifelse(is.na(treat_group_roanu) & (hh_id %in% lst_affected_roanu) & (survey_round==3),1,ifelse(is.na(treat_group_roanu) & !(hh_id %in% lst_affected_roanu) & (survey_round==3),0,treat_group_roanu)))


# PanelView graph
panelView::panelview(1 ~ treat_group_roanu + hh_size + affected_roanu, gridOff = TRUE, by.timing = TRUE,
          data = all_comb, index = c("hh_id","survey_year"), display.all = TRUE, xlab = "Survey year", ylab = "Nbr hh.", pre.post = TRUE, by.unit=TRUE,  background = "white",cex.main = 20, cex.axis= 8, cex.lab = 12, cex.legend = 12, collapse.history = "TRUE", main = "" , legend.labs = c("Control hh.", "Treated hh. (before TC)", "Treated hh. (after TC)"))

ggsave("output/img/stat desc/roanu/attrition_panelview_roanu.jpeg", width = 18, height = 7, units = "cm")
...
# Use with `right_join()` to fill in missing rows (in data)
all_comb <- df_r1_hh %>%
          select(hh_id,survey_round, hh_size, affected_mora,treat_group_mora, survey_year) %>% 
          filter(hh_id %in% lst_menage)%>% 
              mutate(treat_group_mora_post=0) %>% 
          plyr::rbind.fill(
            df_r2_hh %>%
              select(hh_id,survey_round, hh_size, affected_mora,treat_group_mora, survey_year)%>% 
          filter(hh_id %in% lst_menage)%>% 
          ...
          plyr::rbind.fill(
            df_r3_hh %>%
              select(hh_id,survey_round, hh_size, affected_mora,treat_group_mora, survey_year)%>% 
              filter(hh_id %in% lst_menage) %>% 
              mutate(treat_group_mora_post=treat_group_mora))%>% 
  dplyr::right_join(all_comb, by=c("survey_round", "hh_id")) %>% 
  mutate(
    survey_year = ifelse(survey_round==1,2011,
                              ifelse(survey_round==2,2015,
...
# PanelView graph
panelView::panelview(1 ~ treat_group_mora_post + hh_size, gridOff = TRUE, by.timing = TRUE,
          data = all_comb, index = c("hh_id","survey_year"), display.all = TRUE,
          xlab = "Survey year", ylab = "Nbr hh.", pre.post = TRUE, by.unit=TRUE,  background = "white",cex.main = 20, cex.axis= 8, cex.lab = 12, cex.legend = 12, collapse.history = "TRUE", main = "")

ggsave("output/img/stat desc/mora/attrition_panelview_mora.jpeg", width = 18, height = 7, units = "cm")
...
stargazer(re0, re1, re2, re3, re4, re5, type = "html", title = "Attrited hh. after the disaster characteristics at baseline", digits=2, float = TRUE, se.position = "below", omit.stat=c("LL","ser","f","adj.rsq","res.dev"), dep.var.labels.include = TRUE,font.size="small",
se.position = "below",notes.align = "l",table.placement = "H",header = F, column.labels = c("","Exposed UP.", "At least One", "Highly exposed", "Speed","Duration"),
keep = c("hhm_sex" , "hhm_age", "hh_size" ,  "marital_status_hhm", "literacy_hhm", "education_high","hh_head_religion","hh_ethnic_group", "mean_speed_roanu", "mean_time_roanu", "affected_roanu","high_exposed_roanu","exposed_at_least_zero_roanu","nbr_female", "nbr_underfive", "nbr_yngchldrn_5_10", "nbr_teenager_10_20", "nbr_adults_20_65", "nbr_female_underfive", "nbr_female_yngchldrn_5_10", "nbr_female_teenager_10_20", "nbr_female_adults_20_65"), out = paste0("output/table/roanu/attrition_reg_baseline_roanu.html", sep = ""))
...
se.position = "below",notes.align = "l",table.placement = "H",header = F, column.labels = c("","Exposed UP.", "At least One", "Highly exposed", "Speed","Duration"),
# keep = c("hhm_sex" , "hhm_age", "hh_size" ,  "marital_status_hhm", "literacy_hhm", "education_high","hh_head_religion","hh_ethnic_group", "mean_speed_mora", "mean_time_mora", "affected_mora","high_exposed_mora","exposed_at_least_zero_mora","nbr_female", "nbr_underfive", "nbr_yngchldrn_5_10", "nbr_teenager_10_20", "nbr_adults_20_65", "nbr_female_underfive", "nbr_female_yngchldrn_5_10", "nbr_female_teenager_10_20", "nbr_female_adults_20_65"), 
out = paste0("output/table/roanu/attrition_reg_cross_roanu.html", sep = ""))
...
df_global_corrected %>% 
  ...
  mutate(affected_roanu = factor(affected_roanu, levels = c("0", "1"), labels = c("No affected", "Affected"), ordered = TRUE),
    ...
    missing = "ifany"
       ...
       add_p(test=list(all_continuous() ~ "t.test", all_categorical() ~ "fisher.test.simulate.p.values")) %>% 
  modify_header(label ~ "Affected Union (Yes=1,No=0)") %>% 
  add_overall(last=TRUE)  %>% 
  as_flex_table() %>% 
flextable::save_as_docx(
  path = "output/table/roanu/child_int_affected_roanu_stat_desc.docx", 
  prop_section = officer::prop_section(
    page_size = officer::page_size(
      orient = "landscape", # Keep the orientation as landscape 
      width = 10,          # Width aligned to fit a PowerPoint slide
...
df_global_corrected %>% 
  ...
  mutate(exposed_at_least_zero_roanu = factor(exposed_at_least_zero_roanu, levels = c("0", "1"), labels = c("No affected", "Affected"), ordered = TRUE),
    ...
    missing = "ifany"
       ...
       add_p(test=list(all_continuous() ~ "t.test", all_categorical() ~ "fisher.test.simulate.p.values")) %>% 
  modify_header(label ~ "At least one (Yes=1,No=0)") %>% 
  add_overall(last=TRUE) %>% 
  as_flex_table() %>% 
flextable::save_as_docx(
  path = "output/table/roanu/child_int_at_least_one_roanu_stat_desc.docx", 
  prop_section = officer::prop_section(
    page_size = officer::page_size(
      orient = "landscape", # Keep the orientation as landscape 
      width = 10,          # Width aligned to fit a PowerPoint slide
...
df_global_corrected %>% 
  ...
  mutate(high_exposed_roanu = factor(high_exposed_roanu, levels = c("0", "1"), labels = c("No affected", "Affected"), ordered = TRUE),
    ...
    missing = "ifany"
       ...
       add_p(test=list(all_continuous() ~ "t.test", all_categorical() ~ "fisher.test.simulate.p.values")) %>% 
  modify_header(label ~ "Highly exposed (Yes=1,No=0)") %>% 
  add_overall(last=TRUE) %>% 
  as_flex_table() %>% 
flextable::save_as_docx(
  path = "output/table/roanu/child_int_highly_exposed_roanu_stat_desc.docx", 
  prop_section = officer::prop_section(
    page_size = officer::page_size(
      orient = "landscape", # Keep the orientation as landscape 
      width = 10,          # Width aligned to fit a PowerPoint slide
...
df_global_corrected %>% 
  ...
  geom_linerange(aes(ymin = CI2.5, ymax = CI97.5), lwd = 0.3, position = position_dodge(.5)) +
    ...
    theme_light(base_size = 8)+
    theme(legend.position = 'bottom',text = element_text(size=8), legend.direction = "horizontal")
    ggsave("output/img/stat desc/roanu/outcome_mean_affected_roanu.jpeg", width = 18, height = 7, units = "cm")
...
df_global_corrected %>% 
  ...
  geom_linerange(aes(ymin = CI2.5, ymax = CI97.5), lwd = 0.3, position = position_dodge(.5)) +
    ...
    theme_light(base_size = 8)+
    theme(legend.position = 'bottom',text = element_text(size=8), legend.direction = "horizontal")
    ggsave("output/img/stat desc/roanu/outcome_mean_exposed_at_least_roanu.jpeg", width = 18, height = 7, units = "cm")
...
df_global_corrected %>% 
  ...
  geom_linerange(aes(ymin = CI2.5, ymax = CI97.5), lwd = 0.3, position = position_dodge(.5)) +
    ...
    theme_light(base_size = 8)+
    theme(legend.position = 'bottom',text = element_text(size=8), legend.direction = "horizontal")
    ggsave("output/img/stat desc/roanu/outcome_mean_high_exposed_roanu.jpeg", width = 18, height = 7, units = "cm")
...
df_global_corrected %>% 
  ...
  geom_linerange(lwd = 0.3, position = position_dodge(.5)) +
   ...
   facet_wrap(vars(outcome), scales = "free_x") +
    ...
    theme_light(base_size = 8)+
    theme(legend.position = 'bottom',text = element_text(size=8), legend.direction = "horizontal")
      ggsave("output/img/stat desc/roanu/outcome_mean_bysex_affected_roanu_gaps.jpeg", width = 18, height = 7, units = "cm")
...
df_global_corrected %>% 
  ...
  geom_linerange(lwd = 0.3, position = position_dodge(.5)) +
   ...
   facet_wrap(vars(outcome), scales = "free_x") +
    ...
    theme_light(base_size = 8)+
    theme(legend.position = 'bottom',text = element_text(size=8), legend.direction = "horizontal")
      ggsave("output/img/stat desc/roanu/outcome_mean_bysex_exposed_at_least_zero_roanu_gaps.jpeg", width = 18, height = 14, units = "cm")
...
df_global_corrected %>% 
  ...
  geom_linerange(lwd = 0.3, position = position_dodge(.5)) +
   ...
   facet_wrap(vars(outcome), scales = "free_x") +
    ...
    theme_light(base_size = 8)+
    theme(legend.position = 'bottom',text = element_text(size=8), legend.direction = "horizontal")
      ggsave("output/img/stat desc/roanu/outcome_mean_bysex_high_exposed_roanu_gaps.jpeg", width = 18, height = 14, units = "cm")
```
...
plot_df %>% 
    ...
    geom_linerange(lwd = 0.3, position = position_dodge(.5)) +
     ...
     facet_grid(outcome~birth_cohort, scales = "free_x") +
      ...
      theme_light(base_size = 8)+
      theme(legend.position = 'bottom',text = element_text(size=8), legend.direction = "horizontal")
    ggsave("output/img/stat desc/roanu/by_cohort_affected roanu.jpeg", width = 18, height = 14, units = "cm")
...
plot_df %>% 
    ...
    geom_linerange(lwd = 0.3, position = position_dodge(.5)) +
     ...
     facet_grid(outcome~birth_cohort, scales = "free_x") +
      ...
      theme_light(base_size = 8)+
      theme(legend.position = 'bottom',text = element_text(size=8), legend.direction = "horizontal")
    ggsave("output/img/stat desc/roanu/by_cohort_exposed_at_least_zero_roanu.jpeg", width = 18, height = 14, units = "cm")
...
plot_df %>% 
    ...
    geom_linerange(lwd = 0.3, position = position_dodge(.5)) +
     ...
     facet_grid(outcome~birth_cohort, scales = "free_x") +
      ...
      theme_light(base_size = 8)+
      theme(legend.position = 'bottom',text = element_text(size=8), legend.direction = "horizontal")
    ggsave("output/img/stat desc/roanu/by_cohort_high_exposed_roanu.jpeg", width = 18, height = 14, units = "cm")
```
...
df_global_corrected %>% 
  ...
  geom_linerange(aes(ymin = CI2.5, ymax = CI97.5), lwd = 0.3, position = position_dodge(.5)) +
    ...
    theme_light(base_size = 8)+
    theme(legend.position = 'bottom',text = element_text(size=8), legend.direction = "horizontal")
    ggsave("output/img/stat desc/mora/outcome_mean_high_exposed_mora.pdf", width = 18, height = 14, units = "cm")
...
plot_df %>% 
    ...
    geom_linerange(lwd = 0.3, position = position_dodge(.5)) +
     ...
     facet_grid(outcome~birth_cohort, scales = "free_x") +
      ...
      theme_light(base_size = 8)+
      theme(legend.position = 'bottom',text = element_text(size=8), legend.direction = "horizontal")
    ggsave("output/img/stat desc/mora/by_cohort_affected mora.pdf", width = 18, height = 14, units = "cm")
...
plot_df %>% 
    ...
    geom_linerange(lwd = 0.3, position = position_dodge(.5)) +
     ...
     facet_grid(outcome~birth_cohort, scales = "free_x") +
      ...
      theme_light(base_size = 8)+
      theme(legend.position = 'bottom',text = element_text(size=8), legend.direction = "horizontal")
    ggsave("output/img/stat desc/mora/by_cohort_exposed_at_least_zero_mora.pdf", width = 18, height = 14, units = "cm")
...
plot_df %>% 
    ...
    geom_linerange(lwd = 0.3, position = position_dodge(.5)) +
     ...
     facet_grid(outcome~birth_cohort, scales = "free_x") +
      ...
      theme_light(base_size = 8)+
      theme(legend.position = 'bottom',text = element_text(size=8), legend.direction = "horizontal")
    ggsave("output/img/stat desc/mora/by_cohort_high_exposed_mora.pdf", width = 18, height = 14, units = "cm")
```
